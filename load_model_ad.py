    ...
    for epoch in range(args.epochs):
        ...
        for i, (x, *tails) in enumerate(tqdm(train_loader, desc='EPOCH: [%d/%d]' % (epoch + 1, args.epochs))):
            ...
            if args.use_pred:
                ...
                #     data_gen_loss, data_rec_loss, data_pred_loss = model.data_gen_loss(x, y=y, y_pred=y_pred,
                ...
                # if args.use_contrastive:
                #     data_gen_loss, data_rec_loss = model.data_gen_loss(x, y=y, margin=args.contrastive_margin)
                # else:
                if args.use_regularizer:
...
def evaluate(model, test_loader, delay, args):
    ...
    #pred_criterion = WeightedPredictionLoss(args.pred_steps, reduction='none').cuda()
